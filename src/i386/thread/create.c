...
thread_t* thread_create(void (*entry)(), uint8_t is_user, uint32_t prog_size, process_t *process) {
    ...
    if (is_user) {
        ...
        map_page(thread->process->pd_addr, USER_STACK_ADDR, ustack_phys, PAGE_PRESENT | PAGE_WRITE | PAGE_USER);
        ...
        for (uint32_t i = 0; i < pages; i++) {
            ...
            temp_unmap(src);
            temp_unmap(tmp);
            map_page(thread->process->pd_addr, USER_CODE_ADDR + off, phys, PAGE_PRESENT | PAGE_USER);
        }
